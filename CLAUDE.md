...
### 5. 효율적 실행
컨텍스트와 도구 호출을 낭비하지 말 것.
- 이미 읽은 파일은 다시 확인하지 않을 것
- 불필요한 도구 호출을 하지 말 것
- 의존성 없는 도구 호출은 한 번에 동시 실행할 것
- 20줄 이상의 불필요한 출력은 서브에이전트에 위임할 것
- 사용자가 이미 설명한 내용을 다시 반복하지 않을 것
...
7. **마크다운은 qmd로 먼저 검색**: 컨텍스트/문서(`.md`) 파일을 읽기 전에 항상 qmd MCP(`myzip` 컬렉션)로 먼저 검색해 관련 구간을 찾을 것. 코드 파일(`.ts`/`.tsx` 등)은 qmd 인덱싱 대상이 아니므로 Grep을 사용.
